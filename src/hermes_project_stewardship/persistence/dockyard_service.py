    ...
    def report_generate(
        ...
        *,
        ...
        lines = [
            ...
            f"- Verification policy: `{json.dumps(settings['policies'].get('verification', {}), sort_keys=True)}`",
            f"- Release policy: `{json.dumps(settings['policies'].get('release', {}), sort_keys=True)}`",
            f"- Notification policy: `{json.dumps(settings['policies'].get('notification', {}), sort_keys=True)}`",
            "",
            "## Delivery",
            "",
        ...
        report = {
            ...
            "title": title,
            "content": "\n".join(lines).rstrip() + "\n",
            "options": {"include_activity": bool(include_activity)},
